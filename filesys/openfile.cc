...
{
    if (fileSystem != NULL)
        fileSystem->referenceCount[sector]++;
    hdr = new FileHeader;
...
{
    synchDisk->AccquareOpenFile(hdrSector);
    hdr->FetchFrom(hdrSector);
    int result = ReadAt(into, numBytes, seekPosition);
    synchDisk->ReleaseOpenFile(hdrSector);
    seekPosition += result;
    ...
    if ((position + numBytes) > fileLength)
        {
            ...
            hdr->ExtentAllocate(freeMap, numBytes + position - fileLength + 1);
            freeMap->WriteBack(fileSystem->freeMapFile);
            delete freeMap;
            printf("Extend file by %d bytes\n", numBytes + position - fileLength + 1);
            // numBytes = fileLength - position;
